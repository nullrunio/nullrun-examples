"""TC-AR-010 / TC-SDK-014 verification — v3.72 fix.

Sets call_context with tools=['refund_customer'] so the gate's
TB-1 fail-CLOSED branch doesn't shadow Step 7 approval_rule_eval.

Without `set_call_context(tools=...)`, the SDK's pre-flight /gate
returns TOOL_BLOCKED with reason='no_tools_field' BEFORE the approval
rule has a chance to evaluate (orchestrator.rs:1860-1895).

This is the corrected probe.
"""
...
def patched_execute(self, *args, **kwargs):
    # execute takes keyword args: organization_id, execution_id, tool, etc.
    summary = {k: v for k, v in kwargs.items() if k in ('organization_id', 'execution_id', 'tool', 'mode')}
    if args:
        summary['args'] = list(args)[:3]
    print(f'[DEBUG exec_req] {json.dumps(summary, default=str)[:600]}', flush=True)
...
import nullrun
from nullrun import init_or_die, shutdown, handle, set_call_context
...
# CRITICAL: Set the call context to populate the tools field
